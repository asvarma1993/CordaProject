...
@InitiatingFlow
@StartableByRPC
public class AdviceLoCFlow extends FlowLogic<String> {

    private final static Logger logger = LoggerFactory.getLogger(AdviceLoCFlow.class);

    private String locID;
  //  private LoCApplicationState locApplication;

    public AdviceLoCFlow(String locID) {
        this.locID = locID;
      //  this.locApplication = locApplication;
    }
    ...
    public String call() throws FlowException {
        ...
        if (application.size() == 1) {
            ...
            Command command = new Command(new LoCContract.Commands.AdviceLoC(), getOurIdentity().getOwningKey());
            ...
            LoCApplicationState state = new LoCApplicationState(locApplication.getLcNumber(), locApplication.getApplicant(), locApplication.getBeneficiary(),
                    ...
                    locApplication.getDocsRequired(), locApplication.getAdditionalConditions(), locApplication.getComments(), "ADVICED", locApplication.getAmendmentNumber(),
                    Arrays.asList(getOurIdentity(), locApplication.getIssuingBank(), locApplication.getApplicant(), locApplication.getBeneficiary()));
