...
@InitiatingFlow
@StartableByRPC
public class ApplyAmendedLoCFlow extends FlowLogic<String> {

    private final static Logger logger = LoggerFactory.getLogger(ApplyAmendedLoCFlow.class);
 ...
 //   private UniqueIdentifier locID;
    ...
    public ApplyAmendedLoCFlow(String locID, String lcNumber, Party applicant, Party beneficiary, Party issuingBank, Party advisingBank,
    ...
    public String call() throws FlowException {
        ...
        if (application.size() == 1) {
            ...
            logger.info("Making command");
            Command command = new Command(new LoCContract.Commands.ApplyAmendedLoC(), getOurIdentity().getOwningKey());
            LoCApplicationState state = new LoCApplicationState(lcNumber, getOurIdentity(), locApplication.getBeneficiary(),
                    locApplication.getIssuingBank(), locApplication.getAdvisingBank(),
                    locApplication.getLocID(), locApplication.getApplicationCreationDate(), locApplication.getLcType(),
                    locApplication.getApplicationRuleCode(), locApplication.getAppRuleDesc(), locApplication.getApplicantID(), locApplication.getApplicantAddress(),
                    locApplication.getCurrency(), amount, creditAmountTolerance, debitAmountTolerance,
                    issueDate, expiryDate, locApplication.getExpiryPlace(), locApplication.getPresentationDays(),
                    shipmentDate, locApplication.getShipmentPeriod(), locApplication.getIncoTerms(), locApplication.getPartialShipment(),
                    locApplication.getTranshipmentShipment(), portOfLoading, portOfDischarge, locApplication.getPlaceOfTakingInCharge(),
                    locApplication.getFinalDestination(), locApplication.getBeneficiaryName(), locApplication.getBeneficiaryAddress(),
                    ...
                    locApplication.getCreditAvailableWith(), locApplication.getBankName(), locApplication.getBankAddress(), locApplication.getIfscCode(),
                    locApplication.getChargesFrom(), locApplication.getChargesDefaultAccount(), locApplication.getWaiveCharges(), locApplication.getChargeCodeOne(),
                    locApplication.getChargeDebitAccountNum(), locApplication.getChargeCurrency(), locApplication.getChargeAmount(), locApplication.getCommissionCode(),
                    locApplication.getWaiveCommissionFlag(), locApplication.getCommissionPartyCharge(), locApplication.getCommissionCurrency(),
                    locApplication.getCommissionAmount(), locApplication.getCommissionAccount(), locApplication.getMarginRequired(),
                    locApplication.getMarginCalcBase(), locApplication.getMarginPercentage(), locApplication.getMarginDebitAccount(), locApplication.getMarginAmount(),
                    locApplication.getMarginCreditAccount(), locApplication.getDraft(), locApplication.getPoPiContractRegNum(), locApplication.getPoAmount(),
                    locApplication.getPoDate(), locApplication.getDescOfGoods(), locApplication.getDocsRequired(), locApplication.getAdditionalConditions(),
                    locApplication.getComments(), "OPEN",
                    locApplication.getAmendmentNumber(),
